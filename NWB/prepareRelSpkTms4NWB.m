...
rstPttrn = string(baseName)+ " " + sprintf("RW%.2f - %.2f ms ", ...
    ...
    " (unfiltered) *SpkTms.mat";
rstPttrn2 = string(baseName)+ " " + sprintf("RW%.2f-%.2f ", ...
    configStruct.Response_window_s.*1e3) + sprintf("SW%.2f-%.2f ", ...
    configStruct.Spontaneous_window_s.*1e3) + sprintf("VW%.2f-%.2f ms ", ...
    configStruct.Viewing_window_s.*1e3) + string(configStruct.Trigger.Name) + ...
    " (unfiltered) *SpkTms.mat";

rstFiles = arrayfun(@(rst) look4this(rst), [rstPttrn; rstPttrn2], ...
    fnOpts{:}); rstFiles(cellfun(@isempty,rstFiles)) = []; 
rstFiles = cat(1, rstFiles{:}); ciFiles = look4this("cluster_info.tsv");
...
load(expandName(rstFiles(end)), "relativeSpkTmsStruct")
