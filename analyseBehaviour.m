...
% Testing joint amplitude
[~, mu_ctr, sig_ctr] = cellfun(@(c) zscore(sum(c,2), 1), mvFlags(1,:), ...
    fnOpts{:}); mu_ctr = cat(2, mu_ctr{:}); sig_ctr = cat(2, sig_ctr{:});
my_zscore = @(x,m,s) (x - m)./s;
zamp = arrayfun(@(x) arrayfun(@(y) ...
    my_zscore(sum(mvFlags{x,y},2), mu_ctr(y), sig_ctr(y)), ...
    1:size(mvFlags,2), fnOpts{:}), (1:size(mvFlags,1))', fnOpts{:});
