    ...
    if ~isempty(ignVars)
        ...
        if ~isempty(vwVars)
            ...
            while ~vwFlag
                ...
                negVals = configStruct.ViewWindow - abs(windowArray * 1e-3) < 0;
                if sum(negVals(:))
                    fprintf('Please try again...\n')
                    msgStr =...
                        sprintf(...
                        ['The conditioning windows should be within ',...
                        'the viewing window! (%.3f, %.3f ms)'],...
                        1e3 * configStruct.ViewWindow(1),...
                        1e3 * configStruct.ViewWindow(2));
                    waitfor(warndlg(msgStr,'Condition out of bounds','modal'));
                else
                    vwFlag = true;
                end
            end
