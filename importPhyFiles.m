...
if removeNoise
    ...
    index = sortedData2{:,3} == 3;
    sortedData2(index,:) = []; %#ok<NASGU>
    filename = [outputName, '_noNoise_all_channels.mat'];
    fname = fullfile(pathToData, filename);
    save(fname, 'sortedData2', 'fs');
end

% Saves the output file in same as input folder
