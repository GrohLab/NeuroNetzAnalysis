function mergeTriggersbyAnalysis(dataDir)
...
for cfs = 1:Ncf
    ...
    Nvars = double(structfun(@(x) x.npoints,headVars));
    if abs(diff(Nvars)/fs) > 1
        N = max(Nvars);
    else
        N = min(Nvars);
    end
    ...
    offsetSub = round(offset*fs);
    fprintf(1,'Added offset: ---> %.3f ms\n',offset*1e3)
    if existFlags
        if isempty(laser)
            laser = [laser, repmat(min(whisk),1,N + offsetSub)];
        else
            laser = [laser, repmat(min(laser),1,N + offsetSub)];
        end
    else
        whisk = [whisk, repmat(min(whisk),1,N + offsetSub)];
    end
end
