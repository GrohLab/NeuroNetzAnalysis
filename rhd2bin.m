...
% Writing the binary file for Kilosort
fbName = strrep(filename,'.rhd','.bin');
if exist(fbName,'file')
    ovwans = questdlg(...
        'Warning! Binary file exists, would you like to overwrite it?',...
        'Overwrite?','Yes','No','No');
    if strcmp(ovwans,'No')
        fprintf(1,'\nNo file written.\n')
        return
...
num_data_blocks = int64(bytes_remaining / bytes_per_block);
...
fclose(fid);
fclose(fbID);
fprintf(1,'\nSuccessfully wrote binary file %s',fbName);
